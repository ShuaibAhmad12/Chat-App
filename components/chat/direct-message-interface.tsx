...
import { ArrowLeft, MessageCircle, Download, ExternalLink, Users } from "lucide-react"
...
export function DirectMessageInterface({ otherUser, onBack }: DirectMessageInterfaceProps) {
  ...
  // Create a conversation ID for cursor tracking
  const conversationId = currentUserId && otherUser.id ? [currentUserId, otherUser.id].sort().join("_") : undefined

  console.log(
    "DM Interface - Conversation ID:",
    conversationId,
    "Current User:",
    currentUserId,
    "Other User:",
  ...
  // Use cursor tracking for this conversation
  const { cursors } = useCursorTracking({
    conversationId,
    enabled: !!conversationId,
  })

  console.log("DM Interface - Active cursors:", cursors)
  ...
  const handleSendImage = (imageUrl: string, imageName: string) => {
    // Use the proper sendDirectImage function
              ...
              {messages.length === 0 ? (
                ...
                messages.map((message) => {
                  ...
                  return (
                    <div
                      key={message.id}
                      className={`flex gap-3 mb-4 message-animate ${isOwn ? "flex-row-reverse" : "flex-row"}`}
                    >
                      <Avatar className="h-8 w-8 flex-shrink-0 ring-2 ring-primary/10">
                        <AvatarImage
                          src={
                            isOwn ? message.sender.avatar_url || undefined : message.receiver.avatar_url || undefined
                          }
                            ...
                            <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-20 transition-all duration-200 flex items-center justify-center opacity-0 group-hover:opacity-100">
                              <div className="flex gap-2">
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    handleImageDownload(message.image_url!, message.image_name || "image.jpg")
                                  }
                                  className="bg-white/90 hover:bg-white shadow-lg"
                                >
